#!/usr/bin/env python3
# coloc_pipeline_ch1global.py  (with optional Ch2/“green” normalization)
#
...
# - Optional normalization by Ch2 (“green”) per compartment (adds new columns)
# - QA overlays
#
import os
import re
...
def collapse_condition(filename):
    f = filename.lower()
    keywords = {
        "hpg only": "control: hpg only",
        "no hpg": "control: no hpg",
        "chx": "chx",
        "tig": "tig",
        "crb": "crb",
        "oxa": "oxa",
    ...
    found = [label for key, label in keywords.items() if key in f]
    if not found:
        return "unspecified"
    if len(found) == 1:
        return f"{found[0]} only"
    return " + ".join(found)
...
def compute_global_ch1_cutoffs(
    ...
    selem_radius=50,
    dapi_index=2,
    neg_groups=("control: no hpg", "chx + tig","tig + chx"),
    ch1_percentile=99.5,
...
def plot_filtered_visualization(out_png, ch1_show, ch2_show, dapi_mask_rgb, ch1_pos_mask, ch2_pos_mask, ovl_mask):
    ...
    overlay[..., 0] = ch1_pos_mask.astype(float)
    overlay[..., 1] = ch2_pos_mask.astype(float)
...
def main():
    ...
    ap.add_argument("--normalize_by_green", action="store_true",
                    help="Append per-compartment metrics normalized by total Ch2 ('green') intensity.")
    # negatives for CH1
    ...
    for fp in file_paths:
        try:
            ...
            # Per-compartment totals (for green normalization)
            ch2_total_nuc = float(ch2[nuc].sum())
            ch2_total_cyt = float(ch2[cyto].sum())

            # Green-normalized metrics (optional)
            ovl_n_per_green = overlap_intensity_nuc / ch2_total_nuc if args.normalize_by_green and ch2_total_nuc > 0 else np.nan
            ovl_c_per_green = overlap_intensity_cyt / ch2_total_cyt if args.normalize_by_green and ch2_total_cyt > 0 else np.nan
            iwc_n_per_green = (100.0 * overlap_intensity_nuc / (ch2_total_nuc + EPS)) if args.normalize_by_green and ch2_total_nuc > 0 else np.nan
            iwc_c_per_green = (100.0 * overlap_intensity_cyt / (ch2_total_cyt + EPS)) if args.normalize_by_green and ch2_total_cyt > 0 else np.nan
            ...
            # Add Ch2 totals & normalized columns if requested
            if args.normalize_by_green:
                row.update({
                    "Ch2_Total_Nucleus": ch2_total_nuc,
                    "Ch2_Total_Cytoplasm": ch2_total_cyt,
                    "OverlapIntensity_Nucleus_perGreen": ovl_n_per_green,
                    "OverlapIntensity_Cytoplasm_perGreen": ovl_c_per_green,
                    "IWC_Nucleus_perGreen_%": iwc_n_per_green,
                    "IWC_Cytoplasm_perGreen_%": iwc_c_per_green,
                })
    ...
    if "--normalize_by_green" in sum([list(a) if isinstance(a, (list, tuple)) else [a] for a in vars()], []):
        pass  # not reliable; keep explicit with flag below

    if True:
        maybe_norm_cols = [
            "Ch2_Total_Nucleus", "Ch2_Total_Cytoplasm",
            "OverlapIntensity_Nucleus_perGreen", "OverlapIntensity_Cytoplasm_perGreen",
            "IWC_Nucleus_perGreen_%", "IWC_Cytoplasm_perGreen_%",
        ]
        if any("IWC_Nucleus_perGreen_%" in r for r in [row.keys() for row in rows]):
            order_cols[4:4] = maybe_norm_cols
